...
public interface BankAccountRepository extends JpaRepository<BankAccount, UUID> {
    ...
    void sumValueByEmail(@Param("email") String email, @Param("value") double value);

    @Modifying
    @Query("UPDATE BankAccount b SET b.balance = b.balance - :value WHERE b.user.email = :email")
    void subtractValueByEmail(@Param("email") String userEmail, @Param("value") double value);


    @Query("select count(b) > 0 from BankAccount b where b.user.email = :email AND b.balance < :value")
    boolean valueGreaterThanAccountBalance(@Param("email") String userEmail, @Param("value") double value);
}
